...
- **FR-008**: 批量计划：1 次调用输出 {brief, turns[10]{offset,actions,note}}；重规划触发 =
  领土损失 | 战略 sig 变化 | 新消息类型（非白名单）；计划经 `/plan` 内存通道（dashboard 计划面板可查）。
- **FR-009**: 战争分支：任一战（wars 或邻国 war 标志）→ 每回合单次 WAR 提示词调用（前线
  move_army 邻接约束/征兵补充/和谈止损（由 FR-014 交付）），不投资不建设。
...
- **SC-001**: 无突发对局中，10 回合内 LLM 调用次数 ≤ 1（turns.jsonl 的 tokens 计数佐证）。
...
- **SC-005**: 每回合输出 token ≤ 1500（thinking disabled + JSON，对比 v8 出 3198 基线）；输入侧
  前缀缓存命中率 ≥ 80%（provider.last_usage 缓存字段）。
...
- **SC-010**: 资源成本标注全覆盖：ACTION_SPEC 每个动作标注成本类型（查询/金币/行动力/外交点数/
  多资源）零遗漏；批量计划与 WAR 提示词显式包含资源预算/收入约束（提示词可 grep 验收）。
