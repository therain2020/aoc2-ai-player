...
- **FR-019**: 执行前动作消毒（engine-legal 白名单，2026-08-29 真机 T3 灾难后）：move_army 仅接受
  真实前线对（丢弃非法目标）与我方省→我方省集结移动；每回合至多 1 次新宣战；募兵 ≤2 批；
  金<0（破产）时禁募兵；消毒日志随回合记录。
- **FR-020**: 战败止损纪律（2026-08-29 真机 -37 崩盘）：战争分 ≤-20（我方）→ 自动 peace_treaty 止损
  （不等 LLM）；金<0 → 军费不可再扩张（募兵禁 + 提示修复收入）；僵局自动求和（既有 war_cycle）。
- **FR-021**: 驻军数据真实性（2026-08-29 用户纠错：前线 0 假象）：/state 驻军一律用
  `getArmyCivID(civID)` 读取（无越界假 0）；新增 `armies_overview` = 我方每省驻军 {prov, army>0}
  全量清单（部署/集结依据）；战场图可由 `scripts/battle_map.py` 生成。
- **FR-022**: 直观可观测（2026-08-29）：统一状态区（agent/status.py：agent 存活/桥/会话/暂停来源/
  战略档单一快照）供 panel 与 dashboard 共用；暂停文件语义化（#user:dashboard ts / #auto:xxx ts，
  程序绝不自动删除暂停）；battle_map 含前线带+后方驻军省兵力展示。
...
- **SC-011**: 储备纪律可测：连续 10 回合内，金存量降至 <3× 单回合净收入的回合数 ≤ 2
  ...
  必出现重规划或应对动作记录。
- **SC-012**: 战败止损可测（FR-020）：战争分 ≤-20 后 ≤2 回合内出现 peace_treaty 记录；
  金<0 的回合 turn 记录中 recruit 动作数 = 0（消毒生效；turns.jsonl 校验）。
- **SC-013**: 驻军真实精度（FR-021）：/state.armies_overview 省驻军 sum == /state.units 或
  偏差 ≤5%（真机对比；无全 0 假象）；消毒日志：任一决策回合 move_army/declare_war 非法条目
  被丢弃次数可读（turns.jsonl 或 log agg）。
